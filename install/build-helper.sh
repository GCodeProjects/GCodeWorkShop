...
PACKAGE_LANG_EXTRA="cs"
PACKAGE_LANG_LIST="$PACKAGE_LANG_EXTRA $(cat "${SOURCE_DIR}/lang/languages.txt")"
PACKAGE_QT_QM_LIST="qt_ qtbase_"
PACKAGE_VERSION=$(cat ${SOURCE_DIR}/gcodeshared/include/version.h | grep 'define\s*GCODEWORKSHOP_VERSION' | awk -F'"' '{print $(NF-1)}')
...
deploy_dlls()
{
    ...
    cat "$LIB_LIST_FILE"
    # Do nothing if the file LIB_LIST_FILE is empty.
