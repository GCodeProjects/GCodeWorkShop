...
void mergeOneLine(const Diff3Line &d, e_MergeDetails &mergeDetails, bool &bConflict,
...
public:
    MergeResultWindow(QWidget *pParent, OptionDialog *pOptionDialog);
...
private:
    void merge(bool bAutoSolve, int defaultSelector, bool bConflictsOnly = false,
               bool bWhiteSpaceOnly = false);
    ...
    {
    public:
        MergeEditLine(Diff3LineList::const_iterator i, int src = 0)
        {
            m_id3l = i;
            m_src = src;
        ...
        bool isConflict()
        {
            return  m_src == 0 && !m_bLineRemoved && m_str.isNull();
        }

        void setRemoved(int src = 0)
        {
            m_src = src;
            m_bLineRemoved = true;
            m_str = QString();
        ...
        bool isModified()
        {
            return ! m_str.isNull() || (m_bLineRemoved && m_src == 0);
        }

        void setSource(int src, bool bLineRemoved)
        {
            m_src = src;
    ...
    public:
        ...
        void splice(iterator destPos, MergeEditLineList &srcList, iterator srcFirst, iterator srcLast)
        {
            ...
            srcList.setTotalSizePtr(
                0); // Force size-recalc after splice, because splice doesn't handle size-tracking
            setTotalSizePtr(0);
        ...
        {
            if (pTotalSize == 0 && m_pTotalSize != 0) {
                *m_pTotalSize -= size();
            } else if (pTotalSize != 0 && m_pTotalSize == 0) {
                *pTotalSize += size();
            }
    ...
    struct MergeLine {
        ...
        void split(MergeLine &ml2, int
                   d3lLineIdx2)   // The caller must insert the ml2 after this ml in the m_mergeLineList
        {
            ...
            for (MergeEditLineList::iterator i = mergeEditLineList.begin(); i != mergeEditLineList.end();
                    ++i) {
                if (i->id3l() == ml2.id3l) {
                    ml2.mergeEditLineList.splice(ml2.mergeEditLineList.begin(), mergeEditLineList, i,
                                                 mergeEditLineList.end());
                    return;
                }
            }
        ...
        void join(MergeLine
                  &ml2)   // The caller must remove the ml2 from the m_mergeLineList after this call
        {
    ...
    void collectHistoryInformation(int src, Diff3LineList::const_iterator iHistoryBegin,
                                   Diff3LineList::const_iterator iHistoryEnd, HistoryMap &historyMap,
