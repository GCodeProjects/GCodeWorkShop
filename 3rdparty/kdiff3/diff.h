...
struct LineData {
    ...
    {
        pLine = 0;
        pFirstNonWhiteChar = 0;
        size = 0; /*occurances=0;*/ bContainsPureComment = false;
    }

    int width(int tabSize) const;  // Calcs width considering tabs.
    //int occurances;

    bool whiteLine() const
    {
        return pFirstNonWhiteChar - pLine == size;
    }
...
struct Diff3Line {
    ...
    const LineData *getLineData(int src) const
    {
        ...
        if (src == 1 && lineA >= 0) {
            return &m_pDiffBufferInfo->m_pLineDataA[lineA];
        }

        if (src == 2 && lineB >= 0) {
            return &m_pDiffBufferInfo->m_pLineDataB[lineB];
        }

        if (src == 3 && lineC >= 0) {
            return &m_pDiffBufferInfo->m_pLineDataC[lineC];
        }
...
void correctManualDiffAlignment(
    Diff3LineList &d3ll,
...
private:
    ...
    struct FileData {
        FileData()
        {
            m_pBuf = 0;    //m_eLineEndStyle=eLineEndStyleUndefined;
            m_size = 0;
            m_vSize = 0;
            m_bIsText = false;
        }
...
void calcDiff3LineVector(
    Diff3LineList &d3ll,
    Diff3LineVector &d3lv);
...
public:
    ...
    bool isEmpty()
    {
        return firstLine == -1 || (firstLine == lastLine && firstPos == lastPos)
    ...
    int beginPos()
    {
        return firstLine == lastLine ? min2(firstPos, lastPos) :
               firstLine < lastLine ? (firstLine < 0 ? 0 : firstPos) : (lastLine < 0 ? 0 : lastPos);
    }

    int endPos()
    {
        return firstLine == lastLine ? max2(firstPos, lastPos) :
...
int convertToPosInText(
    const QString &s,
    int posOnScreen,
    int tabSize);

int convertToPosOnScreen(
    const QString &s,
    int posInText,
    int tabSize);
...
QString calcHistorySortKey(
    const QString &keyOrder,
    QRegExp &matchedRegExpr,
    const QStringList &parenthesesGroupList);

bool findParenthesesGroups(
    const QString &s,
    QStringList &sl);
