...
void FileAccess::setFile(const QString &name, bool bWantToWrite)
{
    ...
    if (!name.isEmpty()) {
        ...
        if (m_url.isRelative() || !m_url.isValid()
                || bExistsLocal) { // assuming that invalid means relative
...
{
    return m_absoluteFilePath;    // Full abs path
}

QString FileAccess::fileName() const
{
    return m_name;           // Just the name-part of the path, without parent directories
}

QString FileAccess::filePath() const
{
    return m_path;           // The path-string that was used during construction
}
...
bool FileAccess::readFile(void *pDestBuffer, unsigned long maxLength)
{
    if (!m_localCopy.isEmpty()) {
        ...
        if (f.open(QIODevice::ReadOnly)) {
            return interruptableReadFile(f, pDestBuffer,
                                         maxLength);    // maxLength == f.read( (char*)pDestBuffer, maxLength );
        }
    } else if (m_bLocal) {
        QFile f(filePath());
...
QString FileAccess::tempFileName()
{
    ...
    for (int i = 0; ; ++i) {
        ...
        if (! FileAccess::exists(fileName) &&
                QFile(fileName).open(
                    QIODevice::WriteOnly)) { // open, truncate and close the file, true if successful
            break;
        }
    }
...
bool FileAccess::symLink(const QString &linkTarget, const QString &linkLocation)
{
    return 0 ==::symlink(linkTarget.toLocal8Bit().constData(),
                         linkLocation.toLocal8Bit().constData());
    //FileAccessJobHandler fh(0);
...
bool wildcardMultiMatch(const QString &wildcard, const QString &testString, bool bCaseSensitive)
{
    ...
    for (QStringList::Iterator it = sl.begin(); it != sl.end(); ++it) {
        QRegExp pattern(*it, bCaseSensitive ? Qt::CaseSensitive : Qt::CaseInsensitive,
                        QRegExp::Wildcard);
...
bool FileAccessJobHandler::listDir(t_DirectoryList *pDirList, bool bRecursive, bool bFindHidden,
                                   const QString &filePattern,
                                   const QString &fileAntiPattern, const QString &dirAntiPattern, bool bFollowDirLinks,
                                   bool bUseCvsIgnore)
{
    ProgressProxy pp;
    m_pDirList = pDirList;
            ...
            if (searchHandle != INVALID_HANDLE_VALUE) {
                ...
                while (! pp.wasCancelled()) {
                    ...
                    if (fa.m_path != "." && fa.m_path != "..") {
                        ...
                        fa.m_modificationTime = QDateTime(QDate(t.wYear, t.wMonth, t.wDay), QTime(t.wHour, t.wMinute,
                                                          t.wSecond));
                        //FileTimeToLocalFileTime( &findData.ftLastAccessTime, &ft ); FileTimeToSystemTime(&ft,&t);
                        //fa.m_accessTime       = QDateTime( QDate(t.wYear, t.wMonth, t.wDay), QTime(t.wHour, t.wMinute, t.wSecond) );
                        //FileTimeToLocalFileTime( &findData.ftCreationTime, &ft ); FileTimeToSystemTime(&ft,&t);
...
void ProgressDialog::setInformation(const QString &info, double dCurrent, bool bRedrawUpdate)
{
    ...
    int level = static_cast<int>(m_progressStack.size());
...
void ProgressDialog::recalc(bool bUpdate)
{
    ...
    if ((bUpdate && level == 1) || m_t1.elapsed() > 200) {
        if (m_progressStack.empty()) {
            ...
            m_pProgressBar->setValue(int(1000.0 * (i->m_dCurrent * (i->m_dRangeMax - i->m_dRangeMin) +
                                                   i->m_dRangeMin)));
            ++i;

            if (i != m_progressStack.end()) {
                m_pSubProgressBar->setValue(int(1000.0 * (i->m_dCurrent * (i->m_dRangeMax - i->m_dRangeMin) +
                                                i->m_dRangeMin)));
            } else {
