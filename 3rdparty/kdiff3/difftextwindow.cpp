...
void DiffTextWindow::setFirstColumn(int firstCol)
{
    ...
    if (false) { //d->m_pOptionDialog->m_color->m_bRightToLeftLanguage
        deltaX = -deltaX;
...
int DiffTextWindow::getNofColumns()
{
    if (d->m_bWordWrap) {
        ...
        for (int i = 0; i < d->m_size; ++i) {
            if (d->m_pLineData[i].width(100) >
                    nofColumns) {   //d->m_pOptionDialog->m_color->m_bRightToLeftLanguage
                nofColumns = d->m_pLineData[i].width(
                                 d->m_pOptionDialog->m_tabSize);    // d->m_pOptionDialog->m_tabSize
            }
        }
...
int DiffTextWindow::convertLineToDiff3LineIdx(int line)
{
    if (d->m_bWordWrap && d->m_diff3WrapLineVector.size() > 0) {
        return d->m_diff3WrapLineVector[ min2(line,
                                              (int)d->m_diff3WrapLineVector.size() - 1) ].diff3LineIndex;
    } else {
        return line;
    }
}
...
int DiffTextWindow::convertDiff3LineIdxToLine(int d3lIdx)
{
    if (d->m_bWordWrap && d->m_pDiff3LineVector != 0 && d->m_pDiff3LineVector->size() > 0) {
        return (*d->m_pDiff3LineVector)[ min2(d3lIdx,
                                              (int)d->m_pDiff3LineVector->size() - 1) ]->sumLinesNeededForDisplay;
    } else {
        return d3lIdx;
    }
}
...
void DiffTextWindow::setFastSelectorRange(int line1, int nofLines)
{
    ...
    if (isVisible()) {
        int newFirstLine = getBestFirstLine(
                               ...
                               convertDiff3LineIdxToLine(d->m_fastSelectorLine1 + d->m_fastSelectorNofLines) -
                               convertDiff3LineIdxToLine(d->m_fastSelectorLine1),
                               d->m_firstLine,
                               getNofVisibleLines()
                           );
...
void DiffTextWindow::showStatusLine(int line)
{
    ...
    if (d3lIdx >= 0 && d3lIdx < (int)d->m_pDiff3LineVector->size()) {
        ...
        if (pD3l != 0) {
            ...
            QString s = tr("File") + " " + d->m_filename;

            if (l != -1) {
                s += ": " + tr("Line") + " " + QString::number(l + 1);
            } else {
    ...
    if (e->button() == Qt::LeftButton) {
        ...
        if (! s.isEmpty()) {
            ...
            calcTokenPos(s, pos, pos1, pos2,
                         d->m_pOptionDialog->m_tabSize);  //d->m_pOptionDialog->m_color->m_tabSize

            resetSelection();
            d->m_selection.start(line, convertToPosOnScreen(s, pos1,
                                 d->m_pOptionDialog->m_tabSize));    //d->m_pOptionDialog->m_color->m_tabSize
            d->m_selection.end(line, convertToPosOnScreen(s, pos2,
                               d->m_pOptionDialog->m_tabSize));    //d->m_pOptionDialog->m_color->m_tabSize
            update();
...
void DiffTextWindow::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (d->m_selection.firstLine != -1) {
        ...
        if (! false) { //d->m_pOptionDialog->m_color->m_bRightToLeftLanguage
            if (e->x() < d->leftInfoWidth()*fontWidth) {
                deltaX = -1 - abs(e->x() - d->leftInfoWidth() * fontWidth) / fontWidth;
            }

            if (e->x() > width()) {
                deltaX = +1 + abs(e->x() - width()) / fontWidth;
            }
        } else {
            if (e->x() > width() - 1 - d->leftInfoWidth()*fontWidth) {
                deltaX = +1 + abs(e->x() - (width() - 1 - d->leftInfoWidth() * fontWidth)) / fontWidth;
            }

            if (e->x() < fontWidth) {
                deltaX = -1 - abs(e->x() - fontWidth) / fontWidth;
            }
        }

        if (e->y() < 0) {
            deltaY = -1 - sqr(e->y()) / sqr(fm.height());
        }

        if (e->y() > height()) {
            deltaY = +1 + sqr(e->y() - height()) / sqr(fm.height());
        }

        if ((deltaX != 0 && d->m_scrollDeltaX != deltaX) || (deltaY != 0
...
void DiffTextWindow::timerEvent(QTimerEvent *)
{
    ...
    if (d->m_bMyUpdate) {
        ...
        if (d->m_selection.oldLastLine != -1) {
            ...
            if (d->m_selection.oldFirstLine != -1) {
                firstLine = min3(d->m_selection.oldFirstLine, d->m_selection.lastLine,
                                 d->m_selection.oldLastLine);
                lastLine = max3(d->m_selection.oldFirstLine, d->m_selection.lastLine,
                                d->m_selection.oldLastLine);
            } else {
    ...
    if (d->m_scrollDeltaX != 0 || d->m_scrollDeltaY != 0) {
        d->m_selection.end(d->m_selection.lastLine + d->m_scrollDeltaY,
                           d->m_selection.lastPos +  d->m_scrollDeltaX);
...
)
{
    ...
    diffFont.setItalic(false);   //m_pOptionDialog->m_color->m_bItalicForDeltas
    ...
    int fastSelectorLine2 = m_pDiffTextWindow->convertDiff3LineIdxToLine(
                                m_fastSelectorLine1 + m_fastSelectorNofLines) - 1;

    bool bFastSelectionRange = (line >= fastSelectorLine1 && line <= fastSelectorLine2);
    QColor bgColor = m_pOptionDialog->m_bgColor; //m_pOptionDialog->m_color->m_bgColor
    QColor diffBgColor = m_pOptionDialog->m_diffBgColor; //m_pOptionDialog->m_color->m_diffBgColor

    if (bFastSelectionRange) {
        bgColor = m_pOptionDialog->m_currentRangeBgColor; //m_pOptionDialog->m_color->m_currentRangeBgColor
        diffBgColor =
            m_pOptionDialog->m_currentRangeDiffBgColor; //m_pOptionDialog->m_currentRangeDiffBgColor
    }
    ...
    QColor c = m_pOptionDialog->m_fgColor; //m_pOptionDialog->m_fgColor
    p.setPen(c);
    ...
    if (pld != 0) {
        ...
        for (i = wrapLineOffset; i < lineLength; ++i) {
            ...
            if (lineString[i] == '\t') {
                spaces = tabber(outPos, m_pOptionDialog->m_tabSize);   //m_pOptionDialog->m_tabSize
                s[0] = ' ';
            } else {
                s[0] = lineString[i];
            }
            ...
            if (c != m_pOptionDialog->m_fgColor && whatChanged2 == 0
                    && !m_pOptionDialog->m_bShowWhiteSpace) { //m_pOptionDialog->m_fgColor m_pOptionDialog->m_bShowWhiteSpace
                // The user doesn't want to see highlighted white space.
                c = m_pOptionDialog->m_fgColor; //m_pOptionDialog->m_fgColor
            }

            QRect outRect(xOffset + fontWidth * outPos, yOffset, fontWidth * spaces, fontHeight);

            if (false) { //m_pOptionDialog->m_bRightToLeftLanguage
                outRect = QRect(deviceWidth - 1 - (xOffset + fontWidth * outPos), yOffset, -fontWidth * spaces,
                                fontHeight).normalized();
            }

            if (invalidRect.intersects(outRect)) {
                if (!m_selection.within(line, outPos)) {

                    if (c != m_pOptionDialog->m_fgColor) { //m_pOptionDialog->m_fgColor
                    ...
                    if (s[0] == ' '  &&  c != m_pOptionDialog->m_fgColor
                            &&  charChanged[i] != 0) { //m_pOptionDialog->m_fgColor
                        if (m_pOptionDialog->m_bShowWhiteSpaceCharacters
                                && m_pOptionDialog->m_bShowWhiteSpace) { //m_pOptionDialog->m_bShowWhiteSpaceCharacters m_pOptionDialog->m_bShowWhiteSpace
    ...
    p.setPen(m_pOptionDialog->m_fgColor);   //m_pOptionDialog->m_fgColor
    ...
    if (pld != 0) {
        ...
        if (!bWrapLine || wrapLineLength > 0) {
            p.setPen(QPen(m_pOptionDialog->m_fgColor, 0,
                          bWrapLine ? Qt::DotLine : Qt::SolidLine));    //m_pOptionDialog->m_fgColor
            p.drawLine(xOffset + 1, yOffset, xOffset + 1, yOffset + fontHeight - 1);
            p.setPen(QPen(m_pOptionDialog->m_fgColor, 0, Qt::SolidLine));    //m_pOptionDialog->m_fgColor
        }
    }

    if (c != m_pOptionDialog->m_fgColor
            && whatChanged2 == 0) { //&& whatChanged==0 ) //m_pOptionDialog->m_fgColor
        if (m_pOptionDialog->m_bShowWhiteSpace) { //m_pOptionDialog->m_bShowWhiteSpace
            p.setBrushOrigin(0, 0);
            p.fillRect(xLeft, yOffset, fontWidth * 2 - 1, fontHeight, QBrush(c, Qt::Dense5Pattern));
        }
    } else {
        p.fillRect(xLeft, yOffset, fontWidth * 2 - 1, fontHeight,
                   c == m_pOptionDialog->m_fgColor ? bgColor : c);  //m_pOptionDialog->m_fgColor
    }

    if (bFastSelectionRange) {
        p.fillRect(xOffset + fontWidth - 1, yOffset, 3, fontHeight,
                   m_pOptionDialog->m_fgColor); //m_pOptionDialog->m_fgColor
    }
    ...
    for (ci = m_pManualDiffHelpList->begin(); ci != m_pManualDiffHelpList->end(); ++ci) {
        ...
        if (rangeLine1 >= 0 && rangeLine2 >= 0 && srcLineIdx >= rangeLine1
                && srcLineIdx <= rangeLine2) {
            p.fillRect(xOffset - fontWidth, yOffset, fontWidth - 1, fontHeight,
                       m_pOptionDialog->m_manualHelpRangeColor);  //m_pOptionDialog->m_manualHelpRangeColor
            break;
        }
    }
}
...
void DiffTextWindow::paintEvent(QPaintEvent *e)
{
    ...
    //if ( invalidRect.size()==size() )
    {
        ...
        p.setFont(font());
        p.QPainter::fillRect(invalidRect,
                             d->m_pOptionDialog->m_bgColor);   //d->m_pOptionDialog->m_bgColor
...
void DiffTextWindow::print(MyPainter &p, const QRect &, int firstLine, int nofLinesPerPage)
{
    ...
    QColor bgColor = d->m_pOptionDialog->m_bgColor; //d->m_pOptionDialog->m_bgColor
    d->m_pOptionDialog->m_bgColor = Qt::white;
    d->draw(p, invalidRect, p.window().width(), firstLine, min2(firstLine + nofLinesPerPage,
            getNofLines()));
    d->m_pOptionDialog->m_bgColor = bgColor;
    d->m_firstLine = oldFirstLine;
}

void DiffTextWindowData::draw(MyPainter &p, const QRect &invalidRect, int deviceWidth,
                              int beginLine, int endLine)
{
    m_lineNumberWidth = m_pOptionDialog->m_bShowLineNumbers ? (int)log10((
                            double)m_size) + 1 : 0; //m_pOptionDialog->m_bShowLineNumbers

    if (m_winIdx == 1) {
        m_cThis = m_pOptionDialog->m_colorA; //m_pOptionDialog->m_colorA
        m_cDiff1 = m_pOptionDialog->m_colorB; //m_pOptionDialog->m_colorB;
        m_cDiff2 = m_pOptionDialog->m_colorC; //m_pOptionDialog->m_colorC;
    }

    if (m_winIdx == 2) {
        m_cThis = m_pOptionDialog->m_colorB; //m_pOptionDialog->m_colorB;
        m_cDiff1 = m_pOptionDialog->m_colorC; //m_pOptionDialog->m_colorC;
        m_cDiff2 = m_pOptionDialog->m_colorA; //m_pOptionDialog->m_colorA;
    }

    //   if ( m_winIdx==3 )
    //   {
    //      m_cThis = m_pOptionDialog->m_colorC; //m_pOptionDialog->m_colorC;
    //      m_cDiff1 = m_pOptionDialog->m_colorA; //m_pOptionDialog->m_colorA;
    //      m_cDiff2 = m_pOptionDialog->m_colorB; //m_pOptionDialog->m_colorB;
    //   }
    m_cDiffBoth =
        m_pOptionDialog->m_colorForConflict;  // Conflict color m_pOptionDialog->m_colorForConflict
...
QString DiffTextWindowData::getLineString(int line)
{
    if (m_bWordWrap) {
        ...
        return getString(d3LIdx).mid(m_diff3WrapLineVector[line].wrapLineOffset,
                                     m_diff3WrapLineVector[line].wrapLineLength);
    } else {
    ...
    for (it = 0; it < vectorSize; ++it) {
        ...
        if (lineIdx != -1) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                if (lineString[i] == '\t') {
                    spaces = tabber(outPos, d->m_pOptionDialog->m_tabSize);   //d->m_pOptionDialog->m_tabSize
                }
...
bool DiffTextWindow::findString(const QString &s, int &d3vLine, int &posInLine, bool bDirDown,
                                bool bCaseSensitive)
{
    int it = d3vLine;
...
void DiffTextWindow::setSelection(int firstLine, int startPos, int lastLine, int endPos, int &l,
                                  int &p)
{
    d->m_selection.reset();
    ...
    if (lastLine >= getNofLines()) {
        ...
        if (line >= 0) {
            endPos = d->m_pLineData[line].width(
                         d->m_pOptionDialog->m_tabSize);    //d->m_pOptionDialog->m_tabSize
        }
    }
    ...
    if (d->m_bWordWrap && d->m_pDiff3LineVector != 0) {
        ...
        d->m_selection.start(firstWrapLine, convertToPosOnScreen(s1, wrapStartPos,
                             d->m_pOptionDialog->m_tabSize));   //d->m_pOptionDialog->m_tabSize
        d->m_selection.end(lastWrapLine, convertToPosOnScreen(s2, wrapEndPos,
                           d->m_pOptionDialog->m_tabSize));   //d->m_pOptionDialog->m_tabSize
        l = firstWrapLine;
        p = wrapStartPos;
    } else {
        d->m_selection.start(firstLine, convertToPosOnScreen(d->getString(firstLine), startPos,
                             d->m_pOptionDialog->m_tabSize));   //d->m_pOptionDialog->m_tabSize
        d->m_selection.end(lastLine, convertToPosOnScreen(d->getString(lastLine), endPos,
                           d->m_pOptionDialog->m_tabSize));   //d->m_pOptionDialog->m_tabSize
        l = firstLine;
        p = startPos;
    }
...
int DiffTextWindowData::convertLineOnScreenToLineInSource(int lineOnScreen,
        e_CoordType coordType, bool bFirstLine)
{
    int line = -1;

    if (lineOnScreen >= 0) {
        if (coordType == eWrapCoords) {
            return lineOnScreen;
        }
...
{
    if (pFirstLine) {
        *pFirstLine = d->convertLineOnScreenToLineInSource(d->m_selection.beginLine(), coordType,
                      true);
    }
...
void DiffTextWindow::convertSelectionToD3LCoords()
{
    ...
    int firstPosInText = convertToPosInText(s, d->m_selection.beginPos(),
                                            d->m_pOptionDialog->m_tabSize);  //d->m_pOptionDialog->m_tabSize
    convertLineCoordsToD3LCoords(d->m_selection.beginLine(), firstPosInText, firstD3LIdx,
                                 firstD3LPos);

    int lastD3LIdx, lastD3LPos;
    s = d->getLineString(d->m_selection.endLine());
    int lastPosInText = convertToPosInText(s, d->m_selection.endPos(),
                                           d->m_pOptionDialog->m_tabSize);  ///d->m_pOptionDialog->m_tabSize
                                    ...
                                    int nofVisibleColumns)
{
    ...
    if (bWordWrap) {
        ...
        for (i = 0; i < size; ++i) {
            ...
            int linesNeeded = wordWrap(s, nofVisibleColumns,
                                       wrapLineVectorSize == 0 ? 0 : &d->m_diff3WrapLineVector[wrapLineIdx]);
    ...
    if (!d->m_selection.isEmpty() && (!d->m_bWordWrap || wrapLineVectorSize > 0)) {
        ...
        convertD3LCoordsToLineCoords(d->m_selection.beginLine(), d->m_selection.beginPos(), firstLine,
                                     firstPos);

        int lastLine, lastPos;
        convertD3LCoordsToLineCoords(d->m_selection.endLine(), d->m_selection.endPos(), lastLine,
                                     lastPos);

        //d->m_selection.reset();
        d->m_selection.start(firstLine, convertToPosOnScreen(d->getLineString(firstLine), firstPos,
                             d->m_pOptionDialog->m_tabSize)); //d->m_pOptionDialog->m_tabSize
        d->m_selection.end(lastLine, convertToPosOnScreen(d->getLineString(lastLine), lastPos,
                           d->m_pOptionDialog->m_tabSize)); //d->m_pOptionDialog->m_tabSize
    }
}
...
DiffTextWindowFrame::DiffTextWindowFrame(QWidget *pParent, OptionDialog *pOptionDialog,
        int winIdx, QStringList extensions)
    : QWidget(pParent)
{
    ...
    //d->m_pLineEndStyle = new QLabel(tr("Line end style:"));
...
void DiffTextWindowFrame::init()
{
    ...
    if (pDTW) {
        ...
        //d->m_pEncoding->setText( tr("Encoding:") + " " + (pDTW->d->m_pTextCodec!=0 ? pDTW->d->m_pTextCodec->name() : QString()) );
        //d->m_pLineEndStyle->setText( tr("Line end style:") + " " + (pDTW->d->m_eLineEndStyle==eLineEndStyleDos ? tr("DOS") : tr("Unix")) );
    }
}
...
int DiffTextWindow::calcTopLineInFile(int firstLine)
{
    ...
    for (int i = convertLineToDiff3LineIdx(firstLine); i < (int)d->m_pDiff3LineVector->size();
            ++i) {
...
bool DiffTextWindowFrame::eventFilter(QObject *o, QEvent *e)
{
    ...
    if (e->type() == QEvent::FocusIn || e->type() == QEvent::FocusOut) {
        QColor c1 = d->m_pOptionDialog->m_bgColor; //d->m_pOptionDialog->m_bgColor
        QColor c2;

        if (d->m_winIdx == 1) {
            c2 = d->m_pOptionDialog->m_colorA;    //d->m_pOptionDialog->m_colorA;
        } else if (d->m_winIdx == 2) {
            c2 = d->m_pOptionDialog->m_colorB;    //d->m_pOptionDialog->m_colorB;
        }

        //else if ( d->m_winIdx==3 ) c2 = d->m_pOptionDialog->m_colorC; //d->m_pOptionDialog->m_colorC;
