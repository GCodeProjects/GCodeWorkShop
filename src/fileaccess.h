...
public:
    FileAccess();
    // name: local file or dirname or url (when supported)
    FileAccess(const QString &name, bool bWantToWrite = false);
    ~FileAccess();
...
public:
    ...
    bool put(const void *pSrcBuffer, long maxLength, bool bOverwrite, bool bResume = false,
             int permissions = -1);
