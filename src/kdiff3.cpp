...
//   viewToolBar = 0;
   ...
   m_pOptionDialog = new OptionDialog( true, this );
...
//         title = tr("Current Configuration:");
//      }
//      else
//      {
//         s = m_pOptionDialog->parseOptions( args->getOptionList("cs") );
//         title = tr("Config Option Error:");
//      }
//      if (!s.isEmpty())
//      {
//#ifdef _WIN32
...
//   m_bAutoFlag = args!=0  && args->isSet("auto");
    m_bAutoMode = false; //m_bAutoFlag || m_pOptionDialog->m_bAutoSaveAndQuitOnMergeWithoutConflicts;
   if ( m_bAutoMode && m_outputFilename.isEmpty() )
   {
      if ( m_bAutoFlag )
      {
         //KMessageBox::information(this, i18n("Option --auto used, but no output file specified."));
         std::cerr << (const char*)tr("Option --auto used, but no output file specified.").toLatin1() << std::endl;
      }
      m_bAutoMode = false;
   }
   g_pProgressDialog->setStayHidden( true );
...
//   }
   ...
   //autoAdvance->setChecked( true ); //m_pOptionDialog->m_bAutoAdvance
   //showWhiteSpaceCharacters->setChecked( false ); //m_pOptionDialog->m_bShowWhiteSpaceCharacters
   //showWhiteSpace->setChecked( false ); //m_pOptionDialog->m_bShowWhiteSpace
   //showWhiteSpaceCharacters->setEnabled( false ); //m_pOptionDialog->m_bShowWhiteSpace
   //showLineNumbers->setChecked( true ); //m_pOptionDialog->m_bShowLineNumbers
   //wordWrap->setChecked( false ); //m_pOptionDialog->m_bWordWrap
   //if ( ! isPart() )
   {
//  TODO 
...
//       slotViewToolBar();
      ...
      //viewStatusBar->setChecked( false ); //m_pOptionDialog->m_bShowStatusBar
      //slotViewStatusBar();
/*
...
//   }
   ...
   m_bDirCompare = m_bAutoMode = false;
//   if ( m_bAutoFlag && m_bAutoMode && m_bDirCompare )
//   {
//      std::cerr << (const char*)tr("Option --auto ignored for directory comparison.").toLatin1()<<std::endl;
//      m_bAutoMode = false;
//   }
   if (!m_bDirCompare)
   {
      ...
      if ( m_bAutoMode )
      {
         ...
         if ( pSD!=0 )
         {
            ...
            if ( false && fa.exists() ) //m_pOptionDialog->m_bDmCreateBakFiles
            {
