...
int DiffTextWindow::getNofColumns()
{
   ...
   else
   {
      ...
      {
         if ( d->m_pLineData[i].width( 100 ) > nofColumns ) //d->m_pOptionDialog->m_color->m_bRightToLeftLanguage
            nofColumns = d->m_pLineData[i].width(d->m_pOptionDialog->m_tabSize); // d->m_pOptionDialog->m_tabSize
      }
      return nofColumns;
   }
}
...
void DiffTextWindow::mouseDoubleClickEvent( QMouseEvent* e )
{
   ...
   if ( e->button() == Qt::LeftButton )
   {
      ...
      {
         int pos1, pos2;
         calcTokenPos( s, pos, pos1, pos2, d->m_pOptionDialog->m_tabSize); //d->m_pOptionDialog->m_color->m_tabSize

         resetSelection();
         d->m_selection.start( line, convertToPosOnScreen( s, pos1, d->m_pOptionDialog->m_tabSize) ); //d->m_pOptionDialog->m_color->m_tabSize
         d->m_selection.end( line, convertToPosOnScreen( s, pos2, d->m_pOptionDialog->m_tabSize) ); //d->m_pOptionDialog->m_color->m_tabSize
         update();
   ...
   )
{
   ...
   if ( bFastSelectionRange )
   {
      ...
      diffBgColor = m_pOptionDialog->m_currentRangeDiffBgColor; //m_pOptionDialog->m_currentRangeDiffBgColor
   }
   ...
   if (pld!=0)
   {
      ...
      for( i=wrapLineOffset; i<lineLength; ++i )
      {
         ...
         if ( c!= m_pOptionDialog->m_fgColor && whatChanged2==0 && !m_pOptionDialog->m_bShowWhiteSpace ) //m_pOptionDialog->m_fgColor m_pOptionDialog->m_bShowWhiteSpace
         {
         ...
         if ( invalidRect.intersects( outRect ) )
         {
            if( !m_selection.within( line, outPos ) )
            {
               ...
               dto.setPen( c );
               if (s[0]==' '  &&  c!=m_pOptionDialog->m_fgColor  &&  charChanged[i]!=0 ) //m_pOptionDialog->m_fgColor
               {
                  if ( m_pOptionDialog->m_bShowWhiteSpaceCharacters && m_pOptionDialog->m_bShowWhiteSpace) //m_pOptionDialog->m_bShowWhiteSpaceCharacters m_pOptionDialog->m_bShowWhiteSpace
                  {
   ...
   if ( pld!=0 )
   {
      ...
      {
         p.setPen( QPen( m_pOptionDialog->m_fgColor, 0, bWrapLine ? Qt::DotLine : Qt::SolidLine) ); //m_pOptionDialog->m_fgColor
   ...
   if ( c!=m_pOptionDialog->m_fgColor && whatChanged2==0 )//&& whatChanged==0 ) //m_pOptionDialog->m_fgColor
   {
   ...
   {
      p.fillRect( xLeft, yOffset, fontWidth*2-1, fontHeight, c==m_pOptionDialog->m_fgColor ? bgColor : c ); //m_pOptionDialog->m_fgColor
   }

   if ( bFastSelectionRange )
   {
      p.fillRect( xOffset + fontWidth-1, yOffset, 3, fontHeight, m_pOptionDialog->m_fgColor ); //m_pOptionDialog->m_fgColor
   }
...
void DiffTextWindow::paintEvent( QPaintEvent* e )
{
   ...
   {  // double buffering, obsolete with Qt4
      ...
      MyPainter p( this, false, width(), fontMetrics().width('W') ); // For Qt4 change pixmap to this //d->m_pOptionDialog->m_bRightToLeftLanguage

      //p.translate( -invalidRect.x(), -invalidRect.y() );// Remove for Qt4

      p.setFont( font() );
      p.QPainter::fillRect( invalidRect, d->m_pOptionDialog->m_bgColor ); //d->m_pOptionDialog->m_bgColor
...
{
   m_lineNumberWidth = m_pOptionDialog->m_bShowLineNumbers ? (int)log10((double)m_size)+1 : 0; //m_pOptionDialog->m_bShowLineNumbers
...
//   }
   m_cDiffBoth = m_pOptionDialog->m_colorForConflict;  // Conflict color m_pOptionDialog->m_colorForConflict
...
void DiffTextWindow::setSelection( int firstLine, int startPos, int lastLine, int endPos, int& l, int& p )
{
   ...
   if ( lastLine >= getNofLines() )
   {
      ...
      if (line>=0)
         endPos = d->m_pLineData[line].width(d->m_pOptionDialog->m_tabSize); //d->m_pOptionDialog->m_tabSize
   }
   ...
   if ( d->m_bWordWrap && d->m_pDiff3LineVector!=0 )
   {
      ...
      d->m_selection.start( firstWrapLine, convertToPosOnScreen( s1, wrapStartPos, d->m_pOptionDialog->m_tabSize ) ); //d->m_pOptionDialog->m_tabSize
      d->m_selection.end( lastWrapLine, convertToPosOnScreen( s2, wrapEndPos, d->m_pOptionDialog->m_tabSize ) ); //d->m_pOptionDialog->m_tabSize
      l=firstWrapLine;
      p=wrapStartPos;
   }
   else
   {
      d->m_selection.start( firstLine, convertToPosOnScreen( d->getString(firstLine), startPos, d->m_pOptionDialog->m_tabSize ) ); //d->m_pOptionDialog->m_tabSize
      d->m_selection.end( lastLine, convertToPosOnScreen( d->getString(lastLine), endPos, d->m_pOptionDialog->m_tabSize ) ); //d->m_pOptionDialog->m_tabSize
      l=firstLine;
      p=startPos;
   }
   update();
...
void DiffTextWindow::convertSelectionToD3LCoords()
{
   ...
   int firstPosInText = convertToPosInText( s, d->m_selection.beginPos(), d->m_pOptionDialog->m_tabSize ); //d->m_pOptionDialog->m_tabSize
   ...
   int lastPosInText = convertToPosInText( s, d->m_selection.endPos(), d->m_pOptionDialog->m_tabSize ); ///d->m_pOptionDialog->m_tabSize
...
void DiffTextWindow::recalcWordWrap( bool bWordWrap, int wrapLineVectorSize, int nofVisibleColumns )
{
   ...
   if ( !d->m_selection.isEmpty() && ( !d->m_bWordWrap || wrapLineVectorSize>0 ) )
   {
      ...
      //d->m_selection.reset();
      d->m_selection.start( firstLine, convertToPosOnScreen( d->getLineString( firstLine ), firstPos,  d->m_pOptionDialog->m_tabSize)); //d->m_pOptionDialog->m_tabSize
      d->m_selection.end( lastLine, convertToPosOnScreen( d->getLineString( lastLine ),lastPos, d->m_pOptionDialog->m_tabSize)); //d->m_pOptionDialog->m_tabSize
   }
}
...
bool DiffTextWindowFrame::eventFilter( QObject* o, QEvent* e )
{
   ...
   if ( e->type()==QEvent::FocusIn || e->type()==QEvent::FocusOut )
   {
      ...
      QColor c2;
      if      ( d->m_winIdx==1 ) c2 = d->m_pOptionDialog->m_colorA; //d->m_pOptionDialog->m_colorA;
      else if ( d->m_winIdx==2 ) c2 = d->m_pOptionDialog->m_colorB; //d->m_pOptionDialog->m_colorB;
